...
const SCRAPER_DOM_VERSION="2026-09-18-post-reaction-count-scan-v3";
...
async function extractReactions(page:Page,max:number,postUrl:string,warnings:string[]){
  ...
  // LinkedIn has used several overlay implementations for the reaction picker.
  // Use the newly opened overlay as the extraction boundary, not one specific
  // role/class, while never scanning the underlying post.
  const surface=await reactionSurfaceAfterClick(page);
  if(!surface){
    warnings.push("Reaction dialog did not appear after opening the reaction interaction.");
    console.log("Reaction DOM diagnostics: no visible reaction overlay found.");
    return [];
  }
  ...
  for(let round=0;round<30 && reactions.length<max;round++){
    ...
    if(await more.count().catch(()=>0)&&await more.isVisible().catch(()=>false)){
      ...
      await surface.mouse.wheel(0,1200).catch(()=>{});
      await page.waitForTimeout(450);
    }
    if(!added && !(await more.count().catch(()=>0))) break;
  }

  console.log("Reaction DOM diagnostics: overlay profile anchors="+reactions.length);
  if(!reactions.length) warnings.push("Reaction overlay opened, but no public LinkedIn profile URLs were found inside the reaction surface.");
  return reactions;
}
