...
const SCRAPER_DOM_VERSION="2026-09-18-reaction-row-fallback-v5";
...
async function visibleReactionRows(page:Page,max:number){
  ...
  for(let i=0;i<n&&out.length<max;i++){
    ...
    for(let level=0;level<18;level++){
      const t=clean(await node.innerText().catch(()=> ""));
      if(t.length<=500&&reactionRe.test(t)){rowText=t;break;}
      const parent=node.locator("xpath=..");
      if(!(await parent.count().catch(()=>0)))break;
      node=parent;
    }
    if(!rowText)continue;
    const m=rowText.match(reactionRe);
    seen.add(href.toLowerCase());
    out.push({profileUrl:href,name,type:"REACTION",reactionType:m?.[1]?.toUpperCase()||"UNKNOWN"});
  }
  console.log("Reaction row diagnostics: visible reaction rows="+out.length);
  return out;
}
