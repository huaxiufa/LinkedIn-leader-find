...
const SCRAPER_DOM_VERSION="2026-09-18-most-recent-comments-v7";
...
async function selectMostRecentComments(page:Page){
  ...
  await recent.click({timeout:5000}).catch(async()=>{const c=await nearestClickable(recent!);await c.click({timeout:5000,force:true}).catch(()=>{});});
...
async function visibleReactionRows(page:Page,max:number){
  const anchors=page.locator('a[href*="/in/"],a[data-profile-url],a[data-test-profile-url]');
  const n=Math.min(await anchors.count().catch(()=>0),2500);
  const out:ScrapedEngagement[]=[];
  const seen=new Set<string>();
  const reactionRe=/\breacted with\s+(like|love|celebrate|support|insightful|funny)\b/i;
